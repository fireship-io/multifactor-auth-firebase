...
window.recaptchaVerifier = new firebase$1.auth.RecaptchaVerifier('2fa-captcha', {
  size: 'invisible',
...
enrollBtn.onclick = async () => {
  ...
  const phoneAuthProvider = new firebase$1.auth.PhoneAuthProvider();
...
verifyEnrollmentBtn.onclick = async () => {
  ...
  const cred = new firebase$1.auth.PhoneAuthProvider.credential(
    window.verificationId,
    code
  );

  const multiFactorAssertion = firebase$1.auth.PhoneMultiFactorGenerator.assertion(
    cred
  );
